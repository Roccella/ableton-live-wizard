...
- A first selected-clip musical workflow now exists for exact prompt commands: analyze a MIDI clip and expand a 16-beat clip into 32 beats with a deterministic variation intent.
...
- `vary clip resolve` rewrites a selected 16-beat MIDI clip into 32 beats with a firmer ending.
- `vary clip question` rewrites a selected 16-beat MIDI clip into 32 beats with a more open ending.
- `vary clip mini_roll` rewrites a selected 16-beat MIDI clip into 32 beats with a short tail roll.
